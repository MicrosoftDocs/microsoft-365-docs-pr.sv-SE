...
description: I den här artikeln beskrivs hur du använder Azure för att skapa en katastrofåterställningsmiljö för din lokala SharePoint servergrupp.
...
# <a name="sharepoint-server-2013-disaster-recovery-in-microsoft-azure"></a>SharePoint Server 2013 katastrofåterställning i Microsoft Azure
 ...
 Med Azure kan du skapa en katastrofåterställningsmiljö för din lokala SharePoint servergrupp. I den här artikeln beskrivs hur du utformar och implementerar den här lösningen.

 **Titta på översiktsvideo SharePoint över katastrofåterställning i SharePoint Server 2013**
> [!VIDEO https://www.microsoft.com/videoplayer/embed/1b73ec8f-29bd-44eb-aa3a-f7932784bfd9?autoplay=false]
  
 När katastrofen in träffar SharePoint lokala miljön är din högsta prioritet att få igång systemet snabbt igen. Katastrofåterställning med SharePoint går snabbare och enklare när du har en säkerhetskopieringsmiljö som redan körs i Microsoft Azure. Den här videon förklarar huvudbegreppen i en SharePoint av varmt redundans och kompletterar den fullständiga informationen som finns i den här artikeln.
  
Använd den här artikeln med följande lösningsmodell: **SharePoint katastrofåterställning i Microsoft Azure**.
  
[![SharePoint för katastrofåterställning till Azure](../media/SP-DR-Azure.png)](https://go.microsoft.com/fwlink/p/?LinkId=392555)
...
Många organisationer har inte en miljö för katastrofåterställning för SharePoint, vilket kan vara dyra att skapa och underhålla lokalt. Azure-infrastrukturtjänsterna tillhandahåller lockande alternativ för katastrofåterställningsmiljöer som är mer flexibla och billigare än de lokala alternativen.
...
- **Bättre flexibilitet för resurser** Vid en katastrof kan du enkelt skala ut återställnings servergruppen så SharePoint att uppfylla belastningskraven. Skala in när du inte längre behöver resurserna.
...
Mer information om lösningar för katastrofåterställning finns i begreppen Hög tillgänglighet och katastrofåterställning i [SharePoint 2013](/SharePoint/administration/high-availability-and-disaster-recovery-concepts) och Välj en katastrofåterställningsstrategi [för SharePoint 2013.](/SharePoint/administration/plan-for-disaster-recovery)
...
- En återställningsgrupp SharePoint servergrupp i Azure
...
![Element i en SharePoint väntelägeslösning i Azure](../media/AZarch-AZWarmStndby.png)
  
SQL Server-loggleverans med Distributed File System Replication (HURF) används för att kopiera databasbackuper och transaktionsloggar till återställningsfarmen i Azure: 
...
- Du bifogar inte log-shipped SharePoint-innehållsdatabaser i återställningsmiljön förrän en återställningsövning utförs.
...
Den här lösningen handlar inte om en specifik topologi för en SharePoint servergrupp. Fokus för den här lösningen är att använda Azure för redundansfarmen och att implementera loggleverans och TIDER mellan de två miljöerna.
...
På följande bild visas en katastrofåterställningslösning från en lokal SharePoint-servergrupp till en Azure-baserad SharePoint-servergrupp som är konfigurerad som en uppvärmnings- och väntemiljö.
  
**Bild: Topologi och viktiga element i en produktionsfarm och en servergrupp för uppvärmningsåterställning**

![Topologi för en SharePoint servergrupp och en servergrupp för uppvärmningsåterställning](../media/AZarch-AZWarmStndby.png)
...
På följande bild visas en Azure-redundansmiljö där filresursen virtuell dator och den primära SharePoint virtuella databasen körs. Alla andra SharePoint virtuella datorer stoppas. Den virtuella datorn som kör Windows Server Active Directory och DNS visas inte.
  
**Bild: Servergrupp för återställning av isigt vänteläge med virtuella maskiner**

![Element i en SharePoint förkylnings-väntelösning i Azure](../media/AZarch-AZColdStndby.png)
...
Om flera lagringsgrupper implementeras (databaser är utspridda över mer än en uppsättning med SQL Server hög tillgänglighet) måste den primära databasen för varje lagringsgrupp köras för att acceptera loggarna som är kopplade till dess lagringsgrupp.
...
- [Up and Restore of SQL Server Databases](/sql/relational-databases/backup-restore/back-up-and-restore-of-sql-server-databases)
    
- [SharePoint Server 2013 – installation och servergruppsdistribution](/SharePoint/install/installation-and-configuration-overview)
...
Förutom att Windows PowerShell finns det även Windows PowerShell för SQL Server, SharePoint Server och Azure. Kom ihåg att T-SQL, som också kan hjälpa dig att minska tiden för att konfigurera och underhålla din katastrofåterställningsmiljö.
  
## <a name="disaster-recovery-roadmap"></a>Översikt över katastrofåterställning

![Visuell representation av SharePoint över katastrofåterställning.](../media/Azure-DRroadmap.png)
  
Den här översikten förutsätter att du redan har SharePoint servergrupp 2013 distribuerad i produktion.
...
|Fas 4  <br/> |Distribuera servergruppen SharePoint återställning i Azure.  <br/> |
...
Använd vägledningen i Microsoft Azure arkitekturer för [SharePoint 2013](microsoft-azure-architectures-for-sharepoint-2013.md) för att utforma katastrofåterställningsmiljön, inklusive SharePoint återställningsfarmen. Du kan använda grafiken i filen [SharePoint Katastrofåterställning i Azure](https://go.microsoft.com/fwlink/p/?LinkId=392554) Visio för att starta designprocessen. Vi rekommenderar att du utformar hela miljön innan du börjar arbeta i Azure-miljön.
  
Förutom vägledningen i [Microsoft Azure Architectures för SharePoint 2013](microsoft-azure-architectures-for-sharepoint-2013.md) om utformning av virtuellt nätverk, VPN-anslutning, Active Directory och SharePoint-servergruppen ska du se till att lägga till en filresursroll i Azure-miljön.
  
För att det ska gå att skicka loggen i en katastrofåterställningslösning läggs en filresurs virtuella dator till i undernätet där databasrollerna finns. Filresursen fungerar också som den tredje noden i en nodmaitet för tillgänglighetsgruppen SQL Server AlwaysOn. Det här är den rekommenderade konfigurationen för en SharePoint standardgrupp som använder SQL Server AlwaysOn-tillgänglighetsgrupper. 
  
> [!NOTE]
> Det är viktigt att kontrollera förutsättningarna för en databas att delta i en databas SQL Server Tillgänglighetsgrupp för AlwaysOn. Mer information finns i [Förutsättningar, Begränsningar och Rekommendationer för AlwaysOn-tillgänglighetsgrupper.](/sql/database-engine/availability-groups/windows/prereqs-restrictions-recommendations-always-on-availability) 
  
**Bild: Placering av en filserver som används för en katastrofåterställningslösning**

![Visar en filresurs VM tillagd i samma molntjänst som innehåller de SharePoint databasserverrollerna.](../media/AZenv-FSforDFSRandWSFC.png)
  
I det här diagrammet läggs en filresurs virtuell dator till i samma undernät i Azure som innehåller databasserverrollerna. Lägg inte till den virtuella filresursen till en tillgänglighetsuppsättning med andra serverroller, t.ex. SQL Server roller.
  
Om du är orolig för den höga tillgängligheten för loggarna kan du ta en annan metod genom att använda SQL Server säkerhetskopiering och återställning med [Azure Blob Storage Service.](/sql/relational-databases/backup-restore/sql-server-backup-and-restore-with-microsoft-azure-blob-storage-service) Det här är en ny funktion i Azure som sparar loggar direkt till en URL för blob-lagring. Den här lösningen innehåller inte vägledning om hur du använder den här funktionen.
...
Anslut ett lokalt nätverk till ett Microsoft Azure virtuellt nätverk visar hur du planerar och distribuerar det virtuella nätverket i Azure samt hur du skapar [VPN-anslutningen.](connect-an-on-premises-network-to-a-microsoft-azure-virtual-network.md) Följ vägledning i avsnittet för att slutföra följande procedurer:
...
Den här fasen omfattar distribution av både Windows Server Active Directory och DNS till det virtuella nätverket i ett hybridscenario enligt beskrivningen i [Microsoft Azure Architectures for SharePoint 2013](microsoft-azure-architectures-for-sharepoint-2013.md) och som visas i bilden nedan.
  
**Bild: Hybridkonfiguration av Active Directory-domän**

![Två virtuella datorer som distribuerats till det virtuella Azure-nätverket och SharePoint Farm-undernätet är replika domänkontrollanter och DNS-servrar](../media/AZarch-HyADdomainConfig.png)
...
Innan du distribuerar Active Directory i Azure bör du [läsa Riktlinjer för distribution Windows Server Active Directory på virtuella Azure-datorer.](/windows-server/identity/ad-ds/introduction-to-active-directory-domain-services-ad-ds-virtualization-level-100) De här riktlinjerna hjälper dig att avgöra om du behöver en annan arkitektur eller olika konfigurationsinställningar för din lösning.
...
Distribuera servergruppen SharePoint ditt virtuella nätverk enligt dina designplaner. Det kan vara bra att läsa [Planering för SharePoint 2013](/previous-versions/azure/dn275958(v=azure.100)) på Azure-infrastrukturtjänster innan du distribuerar SharePoint roller i Azure.
...
Om du vill konfigurera filreplikering med hjälp av TIDER använder du snapin-modulen DNS Management. Men innan DU gör det loggar du in på den lokala filservern och Azure-filservern och aktiverar tjänsten i Windows.
...
- Öppna **noden Arkiv Storage Tjänster.**
...
|[Teamet Storage Microsoft – Arkivskåpsblogg](https://go.microsoft.com/fwlink/p/?LinkId=392740) <br/> |Blogg om filtjänster och lagringsfunktioner i Windows Server  <br/> |
...
> [!IMPORTANT]
> Log shipping support in SharePoint Server is limited to certain databases. Mer information finns i Alternativ [för hög tillgänglighet och katastrofåterställning för SharePoint (SharePoint 2013).](/SharePoint/administration/supported-high-availability-and-disaster-recovery-options-for-sharepoint-databas) 
...
Om du vill återställa innehållsdatabasen på den sekundära servern så att webbplatserna återges tar du bort alla databasanslutningar före återställningen. Om du vill återställa databasen kör du följande SQL sats.
...
Om du använder värdbaserade webbplatssamlingar, som rekommenderas i Värdnamnad arkitektur och distribution av webbplatssamlingar [(SharePoint 2013)](/SharePoint/administration/host-named-site-collection-architecture-and-deployment)kan du ha flera webbplatssamlingar som ligger i samma webbprogram i SharePoint-servergruppen, med unika DNS-namn (till exempel `https://sales.contoso.com` och `https://marketing.contoso.com` ). I det här fallet kan du skapa DNS-poster för varje webbplatssamling som pekar på kluster-IP-adressen. När en begäran når dina SharePoint-frontend-servrar hanterar de varje begäran till rätt webbplatssamling.
...
|SP-SQL-HA1, SP-SQL-HA2  <br/> |Databasservrar, konfigurerade med SQL Server tillgänglighetsgrupper i 2012 AlwaysOn för att ge hög tillgänglighet. Den här konfigurationen använder SP-SQL-HA1 och SP-SQL-HA2 som de primära och sekundära replikerna.  <br/> |Fyra processorer  <br/> 2–16 GB RAM-minne  <br/> |
...
|:-----|:-----|:-----|:-----|
|C  <br/> |80  <br/> |Systemenhet  <br/> |<DriveLetter>: \\ Program Microsoft SQL Server \\\\  <br/> |
|E  <br/> |80  <br/> |Loggenhet (40 GB)  <br/> |<DriveLetter>: \\ Programfiler \\ Microsoft SQL Server \\ MSSQL10_50.MSSQLSERVER \\ MSSQL \\ DATA  <br/> |
|F  <br/> |80  <br/> |Sida (36 GB)  <br/> |<DriveLetter>: \\ Programfiler \\ Microsoft SQL Server \\ MSSQL \\ DATA  <br/> |
...
|**Enhetsbeteckning**|**Storlek**|**Katalognamn**|**Sökväg**|
|:-----|:-----|:-----|:-----|
|C  <br/> |80  <br/> |Datarotkatalog  <br/> |<DriveLetter>: \\ Program Microsoft SQL Server \\\\  <br/> |
|E  <br/> |500  <br/> |Användardatabaskatalog  <br/> |<DriveLetter>: \\ Programfiler \\ Microsoft SQL Server \\ MSSQL10_50.MSSQLSERVER \\ MSSQL \\ DATA  <br/> |
|F  <br/> |500  <br/> |Användardatabasloggkatalog  <br/> |<DriveLetter>: \\ Programfiler \\ Microsoft SQL Server \\ MSSQL10_50.MSSQLSERVER \\ MSSQL \\ DATA  <br/> |
|G  <br/> |500  <br/> |Temp DB-katalog  <br/> |<DriveLetter>: \\ Programfiler \\ Microsoft SQL Server \\ MSSQL10_50.MSSQLSERVER \\ MSSQL \\ DATA  <br/> |
|H  <br/> |500  <br/> |Temp DB-loggkatalog  <br/> |<DriveLetter>: \\ Programfiler \\ Microsoft SQL Server \\ MSSQL10_50.MSSQLSERVER \\ MSSQL \\ DATA  <br/> |
...
- Distribuera SharePoint grupper
...
#### <a name="deploy-the-sharepoint-farms"></a>Distribuera SharePoint grupper

Våra SharePoint har distribuerats i två steg för att förenkla stabilitet och felsökning vid behov. Under det första steget distribuerades varje servergrupp på det lägsta antalet servrar för varje nivå i topologin för att stödja de nödvändiga funktionerna.
  
Vi skapade databasservrarna med SQL Server installerades innan de SharePoint 2013-servrarna. Eftersom det var en ny distribution skapade vi tillgänglighetsgrupper innan du distribuerade SharePoint. Vi har skapat tre grupper baserade på vägledning för bästa praxis för MCS. 
  
> [!NOTE]
> Skapa platshållardatabaser så att du kan skapa tillgänglighetsgrupper innan SharePoint installationen. Mer information finns i [Konfigurera alltid SQL Server 2012Tillgänglighetsgrupper för SharePoint 2013](/SharePoint/administration/configure-an-alwayson-availability-group)
...
### <a name="the-get-adforest-windows-powershell-command-generates-the-error-the-term-get-adforest-is-not-recognized-as-the-name-of-a-cmdlet-function-script-file-or-operable-program"></a>Kommandot Get-ADForest Windows PowerShell genererar felet", "Termen Get-AD Det kan inte identifieras som namnet på en cmdlet, en funktion, en skriptfil eller ett operabelt program".

När du ska konfigurera användarprofiler behöver du Active Directory-skogens namn. I guiden Lägg till roller och funktioner ser du till att du har aktiverat Active Directory-modulen för Windows PowerShell (under avsnittet Verktyg för fjärrserveradministration **>Rolladministrationsverktyg>AD DS och AD LDS-verktyg).** Kör dessutom följande kommandon innan du använder **Get-AD För** att säkerställa att dina programvaruberoenden läses in.
...
### <a name="sql-server-log-shipping-job-fails-with-access-denied-error-trying-to-connect-to-the-file-share"></a>SQL Server leveransjobbet misslyckas med åtkomstfelet nekad vid försök att ansluta till filresursen
...
### <a name="sql-server-log-shipping-job-indicates-success-but-no-files-are-copied"></a>SQL Server ett leveransjobb för loggen har lyckats, men inga filer kopieras
...
### <a name="managed-metadata-service-or-other-sharepoint-service-fails-to-start-automatically-after-installation"></a>Tjänsten för hanterade metadata (eller SharePoint metadatatjänst) startar inte automatiskt efter installationen

Det kan ta flera minuter att starta tjänsterna, beroende på prestanda och aktuell inläsning av SharePoint Server. Klicka manuellt **på Starta** för tjänsten och ge tillräcklig tid för att starta tjänsten när tjänsten på serverskärmen uppdateras manuellt för att övervaka dess status. Om tjänsten förblir stoppad aktiverar du SharePoint, försöker starta tjänsten igen och kontrollerar sedan loggen efter fel. Mer information finns i Konfigurera [diagnostikloggning i SharePoint 2013](/sharepoint/administration/configure-diagnostic-logging)
