...
---
# <a name="network-performance-recommendations-in-the-microsoft-365-admin-center-preview"></a>Rekommendationer för nätverks prestanda i Microsoft 365 Admin Center (för hands version)

Administrations centret för Microsoft 365 inkluderar nu Live Performance-värden som samlas in från din Microsoft 365-klient organisation och som endast kan visas av administrativa användare i din klient organisation. **nätverks insikter och prestanda rekommendationer** och **nätverks utvärderingar** visas i administrations centret för Microsoft 365 på <https://portal.microsoft.com/adminportal/home#/networkperformance> . Du hittar sidan i navigerings fönstret under **hälsa | Nätverks prestanda**.
...
## <a name="pre-requisites-for-connectivity-measurements-to-appear"></a>Förutsättningar för att anslutnings måtten ska visas

För att anslutnings mätningarna ska visas måste du ha minst två datorer som körs på varje Office-plats som stöder de förutsättningar som krävs. Synkroniseringsklient för OneDrive för företag version 19,232 eller senare måste vara installerat på varje dator. Mer information finns i [versions anteckningar för OneDrive](https://support.office.com/article/onedrive-release-notes-845dcf18-f921-435e-bf28-4e24b95e5fc0).
...
Företag kan förbättra allmän användar upplevelse och skydda sin miljö genom att följa [Office 365-anslutnings principer](https://aka.ms/pnc) och genom att använda funktionen för nätverks prestanda för Microsoft 365 Admin Center. I de flesta fall har dessa allmänna principer en viktig positiv inverkan på slutanvändarens svars tid, tjänstens tillförlitlighet och prestanda i Microsoft 365.
...
## <a name="network-performance-overview-in-the-microsoft-365-admin-center"></a>Översikt över nätverks prestanda i administrations centret för Microsoft 365

Microsoft har befintliga nätverks mått från flera Office-och webb klienter som stöder Microsoft 365-användning. De här måtten används nu för att tillhandahålla design insikter om nätverks arkitektur och en utvärdering av nätverkets prestanda som visas på sidan med **nätverks prestanda** i administrations centret för Microsoft 365.
...
För kunder som vill förbättra noggrannheten för sitespecifika mått och rekommendationer, ger vi mer specifik information. Detta kan minska den ungefärliga placeringen av plats informationen för nätverks måtten. Om du vill redigera adressen för en viss Office-plats <functionality not there yet> .

## <a name="import-undiscovered-office-locations"></a>Importera webbplatser som inte upptäckts

På fliken nätverks prestanda **platser** visas en lista över Office-platser som identifieras automatiskt från Nätverksdiagnostik. Dessa Office-platser upptäcks. Du kan lägga till fler specifika platser inom dessa städer manuellt om de inte automatiskt visas i listan genom att importera dem från en CSV-fil. Om Office-platserna inte visas alls bör du felsöka varför deras nätverks mått inte visas i stället för att bara lägga till platsen här. Du kan också uppdatera adress värdet för befintliga Office-platser.

I CSV-filen aA har Orts platsen en etikett **ort**och en manuellt tillagd Office-plats är märkt **plats**.
...
1. Klicka på länken **Hämta aktuella Office-platser (. csv)** för att exportera den aktuella plats listan till en CSV-fil och spara den på din lokala hård disk. Då får du en korrekt formaterad CSV-fil där du kan lägga till platser. Du kan lämna de exporterade platserna som de är. de dupliceras inte när du importerar den uppdaterade CSV-filen. Om du vill ändra adressen för en befintlig plats uppdateras den när du importerar CSV-filen. Du kan inte ändra adressen för en hittad stad.
1. Öppna CSV-filen och Lägg till platserna genom att fylla i följande fält på en ny rad för varje plats som du vill lägga till. Lämna alla andra fält tomma; värden som du anger i andra fält kommer att ignoreras.
   ...
   1. **Utgående IP-adressintervall 1-5** (valfritt): Ange namnet på kretsen följt av en blankstegsavgränsad lista med giltiga IPv4-eller IPv6-CIDR-adresser för varje intervall. Dessa värden används bara när du har aktiverat SDWAN-integrering för en viss Office-plats.
