---
title: Skapa distributionsgrupper
f1.keywords:
- NOCSH
ms.author: kwekua
author: kwekua
manager: scotv
audience: Admin
ms.topic: article
ms.service: o365-administration
localization_priority: Normal
ms.collection:
- M365-subscription-management
- Adm_O365
- Adm_TOC
search.appverid:
- BCS160
- MET150
- MOE150
ms.assetid: b1ffe755-59e5-4369-826d-825f145a8400
ms.custom:
- seo-marvel-may2020
- AdminSurgePortfolio
- okr_smb
description: Skapa distributionsgrupper eller listor i Microsoft 365 administrationscenter så att du kan skicka e-post till en grupp utan att skriva varje mottagares namn.
ms.openlocfilehash: 7c04ca3e17959afd4138a6ddf9d9cbcd6c4dbafb
ms.sourcegitcommit: 0936f075a1205b8f8a71a7dd7761a2e2ce6167b3
ms.translationtype: MT
ms.contentlocale: sv-SE
ms.lasthandoff: 05/19/2021
ms.locfileid: "52572255"
---
# <a name="create-distribution-groups-in-the-microsoft-365-admin-center"></a>Skapa distributionsgrupper i Microsoft 365 administrationscenter
  
Används när du vill skicka e-post till grupp av personer utan att behöva skriva varje enskild mottagares namn, ordnas distributionsgrupper (distributionslistor) efter ett visst diskussionsämne (till exempel "Marknadsföring") eller av användare som delar gemensamt arbete som kräver att de kommunicerar ofta. De ger dig också ett sätt att automatiskt vidarebefordra e-post till flera e-postadresser.

Distributionsgrupper kallas ibland distributionslistor.
  
## <a name="create-a-distribution-group-list"></a>Skapa en distributionsgrupp (lista)

::: moniker range="o365-worldwide"

Gå till administrationscentret på <a href="https://go.microsoft.com/fwlink/p/?linkid=2024339" target="_blank">https://admin.microsoft.com</a>.

::: moniker-end

::: moniker range="o365-germany"

Gå till administrationscentret på <a href="https://go.microsoft.com/fwlink/p/?linkid=848041" target="_blank">https://portal.office.de/adminportal</a>.

::: moniker-end

::: moniker range="o365-21vianet"

Gå till administrationscentret på <a href="https://go.microsoft.com/fwlink/p/?linkid=850627" target="_blank">https://portal.partner.microsoftonline.cn/adminportal</a>.

::: moniker-end

1. Välj appfönsterikonen  i det övre vänstra hörnet och välj **Admin**.
    
    Hittar du inte appen du söker? I startprogrammet väljer du Alla appar **för att se** en alfabetisk lista över de appar som är tillgängliga för dig. Därifrån kan du söka efter en specifik app. 
    
2. Välj  \> **Gruppera grupper** i det vänstra navigeringsfönstret och välj sedan Lägg till en **grupp**. 
      
3. Välj **Distribution i fältet Välj grupptyp** **och** välj sedan **Nästa**.
  
4. Ange ett namn, en beskrivning och ett e-postalias för den nya gruppen och välj om du vill att personer utanför organisationen ska skicka e-post till gruppen. 
    
5. Tryck **på** Lägg till för att skapa en grupp och granska sedan gruppen och välj **Stäng**. 
    
6. Mer om du vill lägga till användare i distributionslistan finns [i Lägga till en användare eller kontakt i en distributionsgrupp](../email/add-user-or-contact-to-distribution-list.md).
    
Kolla in hur du använder grupp Outlook 2016 Outlook på webben i [Använd kontaktgrupper (tidigare distributionslistor) i Outlook](https://support.microsoft.com/office/1c97fcb2-0ed4-41e6-b401-58f9d7d40e39). 
  
Kolla in [problem med distributionsgruppen](/office365/troubleshoot/groups/distribution-list-issues) för hjälp med distributionslistproblem. 

## <a name="related-content"></a>Relaterat innehåll

[Inställningar för användarens e-post](../email/office-365-user-email-settings.md) (artikel)

[Skapa, redigera eller ta bort en säkerhetsgrupp i Microsoft 365 administrationscenter](../email/create-edit-or-delete-a-security-group.md) (artikel)

[Skapa en delad postlåda](../email/create-a-shared-mailbox.md) (artikel)
