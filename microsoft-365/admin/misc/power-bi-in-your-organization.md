...
### <a name="scenario-1-your-organization-already-has-an-existing-microsoft-365-environment-and-the-user-signing-up-for-power-bi-already-has-an-microsoft-365-account"></a>Scenario 1: Din organisation har redan en befintlig Microsoft 365-miljö och användaren som registrerar sig för Power BI har redan ett Microsoft 365-konto.
...
### <a name="scenario-2-your-organization-has-an-existing-microsoft-365-environment-and-the-user-signing-up-for-power-bi-doesnt-have-an-microsoft-365-account"></a>Scenario 2: Din organisation har en befintlig Microsoft 365-miljö och användaren som registrerar sig för Power BI har inget Microsoft 365-konto.

I det här fallet har användaren en e-postadress i organisationens domän (till exempel contoso.com) men har ännu inte ett Microsoft 365-konto. I det här fallet kan användaren registrera sig för Power BI och automatiskt få ett konto. På så sätt kan användaren komma åt Power BI-tjänsten. Om till exempel en anställd vid namn Nancy använder sin e-postadress för arbete (till exempel Nancy@contoso.com) för att registrera sig, lägger Microsoft automatiskt till Nancy som användare i Contoso Microsoft 365-miljön och aktiverar Power BI för det kontot.
  
### <a name="scenario-3-your-organization-does-not-have-a-microsoft-365-environment-connected-to-your-email-domain"></a>Scenario 3: Din organisation har ingen Microsoft 365-miljö ansluten till din e-postdomän.

Organisationen behöver inte vidta några administrativa åtgärder för att kunna dra nytta av Power BI.
  
> [!IMPORTANT]
> Om din organisation har flera e-postdomäner och du föredrar att alla e-postadresstillägg ska finnas i samma klientorganisation, innan några användare skapar din primära klientorganisation, lägger du till alla e-postadressdomäner till den klienten innan några användare skapar din primära klientorganisation. Det finns ingen automatiserad mekanism för att flytta användare över klienter när de har skapats. Mer information om den här processen finns i [Om jag har flera domäner, kan jag kontrollera den klientorganisation som användare läggs till](#if-i-have-multiple-domains-can-i-control-the-tenant-that-users-are-added-to) i senare i den här artikeln och Lägga till en domän i Office [365](../setup/add-domain.md) online. 
...
Om din organisation redan har en befintlig Microsoft 365-miljö och alla användare i organisationen har Microsoft 365-konton ändras inte identitetshanteringen.
  
Om din organisation redan har en befintlig Microsoft 365-miljö men inte alla användare i organisationen har Microsoft 365-konton skapar vi en användare i klienten och tilldelar licenser baserat på användarens e-postadress för arbete eller skola. Det innebär att antalet användare som du hanterar växer efter hand som användare inom organisationen registrerar sig för tjänsten.
...
Om din organisation inte har en Microsoft 365-miljö ansluten till din e-postdomän kommer det inte att ändras i hur du hanterar identitet. Användarna läggs till i en ny, molnbaserad användarkatalog, och du har möjlighet att välja att ta över som administratör för klientorganisationen och hantera dem.
...
## <a name="if-i-have-multiple-domains-can-i-control-the-tenant-that-users-are-added-to"></a>Om jag har flera domäner, kan jag styra klienten som användare läggs till i?
...
## <a name="how-can-i-prevent-users-from-joining-my-existing-tenant"></a>Hur kan jag hindra användare från att ansluta till min befintliga klient?

Det finns åtgärder du kan vidta som administratör för att förhindra att användare ansluter till din befintliga klientorganisation. Om du blockerar detta misslyckas användarnas försök att logga in och de uppmanas att kontakta organisationens administratör. Du behöver inte upprepa den här processen om du redan har inaktiverat automatisk licensdistribution tidigare (t.ex.
...
Om du vill utföra följande steg måste du installera den senaste 64-bitarsversionen av [Azure Active Directory V2 PowerShell Module](https://www.powershellgallery.com/packages/AzureADPreview/2.0.2.5).
...
> [!NOTE]
> Den här blockeringen förhindrar att nya användare i organisationen registrerar sig för Power BI. Användare som registrerar sig för Power BI innan de inaktiverar nya registreringar för din organisation behåller fortfarande sina licenser. Se [Hur tar jag bort Power BI för användare som redan har registrerat sig?](#how-do-i-remove-power-bi-for-users-that-already-signed-up) 
  
## <a name="how-can-i-allow-users-to-join-my-existing-tenant"></a>Hur tillåter jag användare att gå med i min befintliga klient?
...
2. Leta reda på den användare som du vill ta bort licensen för och välj sedan deras namn.
...
Om du vill skapa den nya vyn följer du stegen till i [Skapa en anpassad användarvy](../add-users/create-edit-or-delete-a-custom-user-view.md#create-a-custom-user-view)i administrationscentret . Under **Tilldelad produktlicens**väljer du **Microsoft Power BI**. När den nya vyn har skapats kan du se alla användare i din klientorganisation som har registrerat sig i det här programmet.
...
Vi valde ett antal som var tillräckligt stort för att majoriteten av organisationerna skulle ha gott om licenser för att ge denna förmån utan dröjsmål till sina användare.
