...
Anvisningar om hur du laddar ned och använder Windows-säkerhet baslinjer för Windows servrar finns [i Windows-säkerhet Originalplan.](/windows/device-security/windows-security-baselines)
...
Du kan Windows Server 2008 R2 SP1, Windows Server 2012 R2 och Windows Server 2016 till Defender för Slutpunkt genom att använda något av följande alternativ:
...
- **Alternativ 3:** [Introduktion Microsoft Endpoint Manager version 2002 och senare](#option-3-onboard-windows-servers-through-microsoft-endpoint-manager-version-2002-and-later)
...
> [!NOTE]
> Defender för fristående slutpunkt för serverlicens krävs per nod för att kunna introducera en Windows-server via Microsoft Monitoring Agent (alternativ 1) eller via Microsoft Endpoint Manager (alternativ 3). Alternativt krävs en Azure Defender för servrar-licens per nod för att kunna hantera en Windows-server via Azure Security Center (alternativ 2) i Funktioner som stöds i [Azure Defender.](/azure/security-center/security-center-services)
...
Du måste installera och konfigurera MMA för Windows att rapportera sensordata till Defender för Slutpunkt. Mer information finns i Samla [in loggdata med Azure Log Analytics-agenten.](/azure/azure-monitor/platform/log-analytics-agent)
...
För Windows Server 2008 R2 SP1 eller Windows Server 2012 R2, kontrollera att du installerar följande snabbkorrigering:
...
För Windows Server 2008 R2 SP1, kontrollera att du uppfyller följande krav:
...
- Installera [antingen .NET framework 4.5](https://www.microsoft.com/download/details.aspx?id=30653) (eller senare) eller [KB3154518](https://support.microsoft.com/help/3154518/support-for-tls-system-default-versions-included-in-the-net-framework)
    ...
    > [!NOTE]
    > Om du hanterar din Windows Server 2008 R2 SP1 med SCCM installerar SCCM-klientagenten .Net Framework 4.5.2. Du behöver därför inte installera .NET Framework 4.5 (eller senare).

För Windows Server 2008 R2 SP1 och Windows Server 2012 R2: Konfigurera och [uppdatera System Center Endpoint Protection klienter.](#configure-and-update-system-center-endpoint-protection-clients)

> [!NOTE]
> Det här steget krävs bara om din organisation använder System Center Endpoint Protection (S SÅD) och du är på Windows Server 2008 R2 SP1 och Windows Server 2012 R2.
...
2. Med nyckeln Arbetsyte-ID och Arbetsyta som hämtas i föregående procedur väljer du någon av följande installationsmetoder för att installera agenten på Windows server:
    - [Installera agenten manuellt med hjälp av konfigurationen](/azure/log-analytics/log-analytics-windows-agents#install-agent-using-setup-wizard). 
    På sidan **Alternativ för konfiguration** av agent väljer Anslut **agenten till Azure Log Analytics (OMS).**
...
### <a name="configure-windows-server-proxy-and-internet-connectivity-settings-if-needed"></a>Konfigurera Windows serverproxy och internetanslutningsinställningar om det behövs
...
- [Konfigurera MMA att använda en proxyserver](/azure/azure-monitor/platform/agent-windows#install-agent-using-setup-wizard)

- [Konfigurera Windows att använda en proxyserver för alla anslutningar](configure-proxy-internet.md)
...
4. Följ introduktionsanvisningarna i [Microsoft Defender](/azure/security-center/security-center-wdatp) för Endpoint med Azure Defender och Om du använder Azure ARC följer du introduktionsanvisningarna i Aktivera Microsoft Defender för [slutpunktsintegrering.](/azure/security-center/security-center-wdatp#enabling-the-microsoft-defender-for-endpoint-integration)

När du har slutfört introduktionsstegen måste du [Konfigurera och uppdatera System Center Endpoint Protection klienter.](#configure-and-update-system-center-endpoint-protection-clients)
...
### <a name="option-3-onboard-windows-servers-through-microsoft-endpoint-manager-version-2002-and-later"></a>Alternativ 3: Windows server via Microsoft Endpoint Manager version 2002 och senare

Du kan onboard Windows Server 2012 R2 och Windows Server 2016 med hjälp Microsoft Endpoint Manager version 2002 och senare. Mer information finns i [Microsoft Defender för slutpunkt i Microsoft Endpoint Manager current branch](/mem/configmgr/protect/deploy-use/defender-advanced-threat-protection).
...
> [!NOTE]
>
> - Onboarding-paketet för Windows Server 2019 till Microsoft Endpoint Manager för närvarande ett skript. Mer information om hur du distribuerar skript i Configuration Manager finns i [Paket och program i Configuration Manager.](/configmgr/apps/deploy-use/packages-and-programs)
...
Stöd för Windows Server ger djupare insikter i serveraktiviteter, täckning för kernel- och minnesangreppsidentifiering och möjliggör svarsåtgärder.

1. Konfigurera onboardinginställningar för Defender för slutpunkt på Windows server med samma verktyg och metoder för Windows 10 enheter. Mer information finns i Onboard [Windows 10 enheter.](configure-endpoints.md)
...
3. Kör följande kommando för att kontrollera om Microsoft Defender AV är installerat:
   ...
   ```sc.exe query Windefend```
    ...
    Om resultatet är "Den angivna tjänsten finns inte som en installerad tjänst" måste du installera Microsoft Defender AV. Mer information finns i [Microsoft Defender Antivirus i Windows 10](/windows/security/threat-protection/microsoft-defender-antivirus/microsoft-defender-antivirus-in-windows-10).
...
Defender för Endpoint kan integreras med Azure Defender för att tillhandahålla en Windows lösning för serverskydd. Med den här integreringen kan Azure Defender använda kraften i Defender för Endpoint för att tillhandahålla förbättrad identifiering av hot för Windows-servrar.

Följande funktioner ingår i den här integreringen:

- Automatisk onboarding – Defender för slutpunkts sensor aktiveras automatiskt på Windows-servrar som är onboarded to Azure Defender. Mer information om Azure Defender-registrering finns i Använda [den integrerade Microsoft Defender för Slutpunktslicensen.](/azure/security-center/security-center-wdatp)
...
- [Konfigurera medlemskap i S FLERA-klientens Molnskyddstjänst](/windows/security/threat-protection/microsoft-defender-antivirus/enable-cloud-protection-microsoft-defender-antivirus) till **inställningen** Avancerat.

## <a name="offboard-windows-servers"></a>Offboard Windows servrar

Du kan offboard Windows Server (SACK), Windows Server 2019 och Windows Server 2019 Core Edition på samma sätt som för Windows 10-klientenheter.

För andra Windows-serverversioner har du två alternativ för att Windows offboardservrar från tjänsten:

- Avinstallera MMA-agenten
- Ta bort Defender för slutpunktsarbetsytans konfiguration

> [!NOTE]
> Offboarding gör att Windows-servern slutar skicka sensordata till portalen, men data från Windows-servern, inklusive referens till aviseringar som den hade, kommer att behållas i upp till 6 månader.
...
Om du vill ta Windows-servern kan du avinstallera MMA-agenten från Windows-servern eller koppla från den från rapporteringen till Defender för slutpunkt-arbetsytan. När agenten har varit offboarding skickar Windows inte längre sensordata till Defender för Slutpunkt.
Mer information finns i Inaktivera [en agent](/azure/log-analytics/log-analytics-windows-agents#to-disable-an-agent).
...
Om du vill ta Windows server kan du använda någon av följande metoder:
...
För de två KBs (en för Windows Server 2008R2/Windows 7 och en annan för Windows Server 2012 R2) upprepar du processen men skapar objektnivåanpassning på fliken COMMON, så filen kopieras bara till rätt version av plattformen/operativsystemet i omfattning:

:::image type="content" source="images/targeteditor.png" alt-text="målredigeraren":::

- För Windows Server 2008 R2 behöver du (och den kommer bara att kopieras) Windows6.1-BJ3080149-x64.msu
...
För Windows Server 2008 R2 PS1, kontrollera att du uppfyller följande krav:
...
Kontrollera vilka KB som finns innan du börjar använda Windows Server 2008 R2 Med den här processen kan du hantera alla servrar om du inte har Konfigurationshanteraren som hanterar servrar.
