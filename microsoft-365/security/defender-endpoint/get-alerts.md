...
<br>Operatorer som stöds av OData:
<br>```$filter``` på: ```alertCreationTime``` ```lastUpdateTime``` , , , , ```incidentId``` ```InvestigationId``` och ```status``` ```severity``` ```category``` egenskaper.
<br>```$top``` med maxvärde på 10 000
<br>```$skip```
<br>```$expand``` av ```evidence```
