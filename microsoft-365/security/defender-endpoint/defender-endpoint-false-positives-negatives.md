...
---
# <a name="address-false-positivesnegatives-in-microsoft-defender-for-endpoint"></a>Adressera falska positiva/negativa tal i Microsoft Defender för Endpoint
...
Du kan också få hjälp om du fortfarande har problem med [falska positiva/negativa](#still-need-help) när du har utfört de uppgifter som beskrivs i den här artikeln.

![Steg för att åtgärda falska positiva och negativa tal](images/false-positives-step-diagram.png)

> [!NOTE]
> Den här artikeln är avsedd som vägledning för säkerhetsoperatorer och säkerhetsadministratörer som använder [Microsoft Defender för Slutpunkt.](microsoft-defender-advanced-threat-protection.md)
...
3. Välj en avisering om du vill ha mer information om aviseringen. (Se [Granska aviseringar](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/review-alerts).)
...
> [!TIP]
> Behöver du hjälp med regelregler? Mer [information finns i Ignorera en avisering och skapa en ny regel för en ny regel.](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/manage-alerts#suppress-an-alert-and-create-a-new-suppression-rule)
...
I allmänhet behöver du inte definiera undantag för Microsoft Defender Antivirus. Se till att du definierar undantag sparsamt och att du endast tar med filer, mappar, processer och process öppna filer som resulterar i falska positiva resultat. Kontrollera även regelbundet att du har definierat undantag. Vi rekommenderar att du [använder Microsoft Endpoint Manager](https://docs.microsoft.com/mem/endpoint-manager-overview) för att definiera eller redigera dina undantag för antivirus. Du kan emellertid använda andra metoder, till exempel [Grupprincip](https://docs.microsoft.com/azure/active-directory-domain-services/manage-group-policy) (se [Hantera Microsoft Defender för slutpunkt).](manage-atp-post-migration.md)

> [!TIP]
> Behöver du hjälp med undantag för antivirusprogram? Se [Konfigurera och validera undantag för Microsoft Defender Antivirus-genomsökningar.](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-antivirus/configure-exclusions-microsoft-defender-antivirus)
...
7. Om du **använder omfattningstaggar** i organisationen på fliken Omfattningstaggar anger du omfattningstaggar för principen du skapar. (Se [Omfattningstaggar](https://docs.microsoft.com/mem/intune/fundamentals/scope-tags).)
8. På **fliken Tilldelningar** anger du de användare och grupper som principen ska tillämpas på och väljer sedan **Nästa.** (Om du behöver hjälp med tilldelningar kan du gå [till Tilldela användar- och enhetsprofiler i Microsoft Intune](https://docs.microsoft.com/mem/intune/configuration/device-profile-assign).)
...
Om du skapar en indikator för "tillåt" för en [fil,](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/indicator-file)till exempel en körbar fil, förhindrar det att filer som din organisation använder blockeras. Filer kan innehålla flyttbara körbara filer (PE), till exempel `.exe` `.dll` filer. 

Innan du skapar indikatorer för filer bör du kontrollera att följande krav uppfylls:
- Microsoft Defender Antivirus är konfigurerat med molnbaserat skydd aktiverat (se [Hantera molnbaserat skydd)](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-antivirus/deploy-manage-report-microsoft-defender-antivirus)
...
- Funktionen [Blockera eller tillåt är aktiverad](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/advanced-features) 

#### <a name="indicators-for-ip-addresses-urls-or-domains"></a>Indikatorer för IP-adresser, URL-adresser och domäner

Om du skapar en "tillåt"-indikator för en [IP-adress, URL-adress](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/indicator-ip-domain)eller domän förhindrar det att webbplatser eller IP-adresser som din organisation använder blockeras.

Innan du skapar indikatorer för IP-adresser, URL:er eller domäner bör du kontrollera att följande krav uppfylls:
- Nätverksskydd i Defender för Slutpunkt är aktiverat i blockläge (se [Aktivera nätverksskydd](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/enable-network-protection))
- Antimalware client version is 4.18.1906.x or later 
- Enheter kör Windows 10, version 1709 eller senare 

Anpassade nätverksindikatorer är aktiverat i Microsoft Defender Säkerhetscenter (se [Avancerade funktioner)](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/advanced-features)   

#### <a name="indicators-for-application-certificates"></a>Indikatorer för programcertifikat 

När du [skapar en "tillåt"-indikator](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/indicator-certificates)för ett programcertifikat förhindrar det att program, till exempel internt utvecklade program, som din organisation använder blockeras. `.CER` eller `.PEM` så stöds filnamnstillägg.   
...
1. Läs riktlinjerna här: [Skicka filer för analys](https://docs.microsoft.com/windows/security/threat-protection/intelligence/submission-guide).
...
3.  Läs riktlinjerna här: [Skicka filer för analys](https://docs.microsoft.com/windows/security/threat-protection/intelligence/submission-guide).
...
> [!TIP]
> Mer information finns i [Skicka filer för analys.](https://docs.microsoft.com/windows/security/threat-protection/intelligence/submission-guide#how-does-microsoft-prioritize-submissions)
...
> [!TIP]
> Mer information om hur du konfigurerar ditt moln levererat skydd finns [i Ange skyddsnivån för moln levererat.](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-antivirus/specify-cloud-protection-level-microsoft-defender-antivirus)
...
6. Om du **använder omfattningstaggar** i organisationen på fliken Omfattningstaggar anger du omfattningstaggar för principen. (Se [Omfattningstaggar](https://docs.microsoft.com/mem/intune/fundamentals/scope-tags).)
8. På **fliken Tilldelningar** anger du de användare och grupper som principen ska tillämpas på och väljer sedan **Nästa.** (Om du behöver hjälp med tilldelningar kan du gå [till Tilldela användar- och enhetsprofiler i Microsoft Intune](https://docs.microsoft.com/mem/intune/configuration/device-profile-assign).)
9. Granska **inställningarna på fliken** Granska + skapa och välj sedan **Skapa**.  
...
> [!TIP]
> Mer information om PUA finns i Identifiera [och blockera potentiellt oönskade program.](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-antivirus/detect-block-potentially-unwanted-apps-microsoft-defender-antivirus)
...
7. På **fliken Tilldelningar** anger du de användare och grupper som principen ska tillämpas på och väljer sedan **Nästa.** (Om du behöver hjälp med tilldelningar kan du gå [till Tilldela användar- och enhetsprofiler i Microsoft Intune](https://docs.microsoft.com/mem/intune/configuration/device-profile-assign).)
...
Åtgärder vidtas [på](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/automation-levels) artefakter som anses vara skadliga eller misstänkta, beroende på hur mycket automatisering som har ställts in för din organisation och *andra* *säkerhetsinställningar.* I vissa fall sker åtgärder automatiskt. i andra fall vidtas åtgärder manuellt eller bara efter godkännande från säkerhetsteamet. 

- [Läs mer om automatiseringsnivåer](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/automation-levels)– och sedan 
- [Konfigurera AIR-funktioner i Defender för Slutpunkt](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/configure-automated-investigations-remediation).
