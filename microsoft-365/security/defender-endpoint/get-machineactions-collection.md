...
Hämtar en samling [datoråtgärder.](machineaction.md)
<br>Stöder [OData V4-frågor.](https://www.odata.org/documentation/)
<br>OData-frågan ```$filter``` stöds för: ```status``` , ```machineId``` ```type``` , ```requestor``` och ```creationDateTimeUtc``` egenskaper.
...
Om det lyckas returnerar den här metoden 200, OK-svarskod med en samling [machineAction-enheter.](machineaction.md)
...
Här är ett exempel på begäran för en organisation som har tre maskinåtgärder.
...
Här är ett exempel på en begäran som filtrerar MachineActions efter maskin-ID och visar de senaste två MachineActions.
