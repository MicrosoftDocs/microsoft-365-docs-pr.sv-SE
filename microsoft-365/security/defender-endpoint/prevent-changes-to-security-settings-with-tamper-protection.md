...
Skydd mot manipulering låser Microsoft Defender Antivirus skydd mot säkerhet, standardvärden och förhindrar att säkerhetsinställningarna ändras via appar och metoder som:
...
- När du använder Microsoft Defender Säkerhetscenter för att hantera skydd mot manipulering behöver du inte använda antingen Intune eller klientorganisationens metod för att bifoga.
...
Om du använder Windows Server 2016, Windows 10 version 1709, 1803 eller [1809](/windows/release-health/status-windows-10-1809-and-windows-server-2019)visas  inte Manipuleringsskydd i Windows-säkerhet-appen. I stället kan du använda PowerShell för att avgöra om skydd mot manipulering är aktiverat. 
   
I Windows Server 2016 ändras inte Inställningar-appen korrekt återspeglar statusen för realtidsskydd när skydd mot manipulering har aktiverats.
...
3. Leta efter eller i `IsTamperProtected` `RealTimeProtectionEnabled` resultatlistan. (Värdet sant innebär *att* skydd mot manipulering har aktiverats.)
...
2. Gå till [Microsoft Endpoint Manager Säkerhetsantivirusprogram](https://go.microsoft.com/fwlink/?linkid=2109431)för **slutpunkt i administrationscentret** och välj sedan + Skapa  >   **princip.** 
