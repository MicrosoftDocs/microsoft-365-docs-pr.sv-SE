...
I Microsoft 365-organisationer med postlådor i Exchange Online eller fristående EOP-organisationer (Exchange Online Protection) utan Exchange Online-postlådor använder EOP principer för skräppostskydd (kallas även principer för skräppostfilter eller innehållfilterprinciper) för att söka igenom inkommande meddelanden efter skräppost och massutskick (kallas även grå e-post). Mer information finns i [Konfigurera principer för skräppostskydd i EOP](configure-your-spam-filter-policies.md).

Om du vill ha fler alternativ för att filtrera massutskick kan du skapa e-postflödesregler (kallas även transportregler) för att söka efter textmönster eller fraser som ofta förekommer i massutskick och markera dessa meddelanden som skräppost. Mer information om massutskick finns i Vad är skillnaden mellan skräppost och massutskick [av](what-s-the-difference-between-junk-email-and-bulk-email.md) e-post? och nivå för massklagomål [(BCL) i EOP.](bulk-complaint-level-values.md)

I det här avsnittet förklaras hur du skapar de här e-postflödesreglerna i administrationscentret för Exchange (EAC) och PowerShell (Exchange Online PowerShell för Microsoft 365-organisationer med postlådor i Exchange Online, fristående EOP PowerShell för organisationer utan Exchange Online-postlådor).
...
- Listan med ord och textmönster som används för att identifiera massutskick i exemplen är inte uttömmande. kan du lägga till och ta bort poster efter behov. Men de är en bra utgångspunkt.

- Sökningen efter ord eller textmönster i ämneshuvudet eller andra  rubrikfält i meddelandet inträffar när meddelandet har avkodats från kodningsmetoden för MIME-innehållsöverföring som användes för att överföra det binära meddelandet mellan SMTP-servrar i ASCII-text. Du kan inte använda villkor eller undantag för att söka efter rådata (vanligtvis Base64) kodade värden för ämnesfältet eller andra rubrikfält i meddelanden.

- Följande procedurer markerar ett massmeddelande som skräppost för hela organisationen. Du kan dock lägga till ytterligare ett villkor för att använda de här reglerna endast för specifika mottagare, så att du kan använda aggressiv filtrering för några få användare med hög målgrupp, medan övriga användare (som oftast får den massutskick som de registrerat sig för) inte påverkas.
...
3. Konfigurera **följande inställningar** på sidan Ny regel som öppnas:
   ...
   - **Använd den här regeln om:** Konfigurera någon av följande inställningar för att söka efter innehåll i meddelanden med hjälp av reguljära uttryck (RegEx) eller ord eller fraser:
     ...
     - **Ämne eller brödtext** \> Ämne eller brödtext matchar dessa textmönster: I dialogrutan Ange ord eller fraser som  visas anger du något av följande värden, klickar på Lägg till ikon och upprepar **tills** du har angett alla  ![ ](../../media/ITPro-EAC-AddIcon.png) värden.
     ...
     - **Ämne eller brödtext** \> **Ämne eller** brödtext innehåller något av följande ord: I dialogrutan Ange ord eller  fraser som visas anger du något av följande värden, klickar på Lägg till ikon och upprepar **tills** du har angett ![ alla ](../../media/ITPro-EAC-AddIcon.png) värden.
   ...
   - **Gör så här:** Välj **Ändra meddelandeegenskaperna** \> **för att ange SCL (Spam Confidence Level).** I dialogrutan **Ange SCL** som visas konfigurerar du en av följande inställningar:

     - Om du vill markera meddelanden **som skräppost** väljer du **6.** Den åtgärd som du har  konfigurerat för skräppostfiltrering som avgörs i principerna för skräppostskydd tillämpas på meddelandena (standardvärdet är Flytta meddelandet till mappen **Skräppost).**

     - Markera meddelanden som skräppost **med hög konfidens** genom att **välja 9.** Åtgärden som du har konfigurerat  för skräppostfiltrering med hög konfidens i principerna för skydd mot skräppost tillämpas på meddelandena (standardvärdet är Flytta meddelandet till mappen **Skräppost).**
...
Använd följande syntax för att skapa en eller båda av e-postflödesreglerna (reguljära uttryck kontra ord):

```powershell
New-TransportRule -Name "<UniqueName>" [-SubjectOrBodyMatchesPatterns "<RegEx1>","<RegEx2>"...] [-SubjectOrBodyContainsWords "<WordOrPhrase1>","<WordOrPhrase2>"...] -SetSCL <6 | 9>
```

I det här exemplet skapas en ny regel med namnet "Massfiltrering – RegEx" som använder samma lista med reguljära uttryck från tidigare i avsnittet för att ange meddelanden som **skräppost.**
...
I det här exemplet skapas en ny regel med namnet "Massfiltrering – Ord" som använder samma lista med ord från tidigare i avsnittet för att ange skräppost med **hög konfidens.**
...
- Gå till E-postflödesregler **i E-postflöde** och välj regeln genom att klicka \>  \> på \>  ![ ](../../media/ITPro-EAC-EditIcon.png) redigera-ikonen och kontrollera inställningarna.

- I PowerShell ersätter \<Rule Name\> du med namnet på regeln och kör följande kommando för att verifiera inställningarna:

  ```powershell
  Get-TransportRule -Identity "<Rule Name>" | Format-List
  ```

- Skicka ett testmeddelande från ett externt konto till en mottagare som innehåller någon av fraserna eller textmönstren och verifiera resultatet.
