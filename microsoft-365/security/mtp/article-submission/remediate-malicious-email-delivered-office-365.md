...
---
# <a name="remediate-malicious-emails-that-were-delivered-in-office-365"></a>Åtgärda skadliga e-postmeddelanden som levererades i Office 365
...
> [!NOTE]
> Hoppa till en undersökning artikel [här](https://docs.microsoft.com/microsoft-365/security/office-365-security/investigate-malicious-email-that-was-delivered?view=o365-worldwide).

## <a name="what-you-need-to-know-before-you-begin"></a>Vad du behöver veta innan du börjar

> [!NOTE]
> För att säkerhetsteam ska kunna åtgärda e-postmeddelanden måste de ha en sök- och rensningsroll tilldelad till dem. Rolltilldelning görs med behörigheter i säkerhets- och efterlevnadscenter. <p>

## <a name="remediations-through-manual-investigation-or-air"></a>Avhjälpande åtgärder genom manuell undersökning eller AIR
...
När e-postmeddelanden har valts från Threat Explorer kan skapande av reparation börja med att vidta direkta åtgärder eller genom att köa e-postmeddelanden för en åtgärd:
 ...
 - Direkt godkännande: När åtgärder som "Flytta till inkorgen", "Flytta till skräp", "Flytta till borttagna objekt", "Mjuk borttagning", "Hård borttagning" väljs av säkerhetspersonal med lämpliga behörigheter och nästa steg följs tills reparation skapas, börjar reparationsprocessen att utföra en markerad åtgärd. Ett tillfälligt utfällbart utfällbart alternativ visar pågående reparation.

 - Två steg godkännande: "Lägg till reparation" åtgärd kan vidtas av en administratör som inte har rätt behörighet eller som behöver vänta längre för att utföra åtgärden. I det här fallet utförs inte reparationsåtgärden direkt. I stället läggs e-postmeddelanden till i en reparationsbehållare som måste godkännas för att köras. Tills reparationen har godkänts kommer e-postmeddelandena inte att åtgärdas. När reparationen har godkänts kommer åtgärder att vidtas på e-postmeddelandena.

**Automatiserade undersökningar och svar (AIR)** utlöses av matchande kriterier i systemet (ZAP, användare rapporterade etc.) eller av säkerhetsoperationer team inifrån Threat Explorer genom att manuellt plocka e-postmeddelanden som ska undersökas. Dessa undersökningar kan omfatta vissa rekommenderade åtgärder som måste godkännas av säkerhetsoperationsgrupper. Dessa reparationsåtgärder visas på fliken Åtgärd i den automatiska undersökningen.  
...
Alla avhjälpningar (antingen direkt godkännande eller tvåstegsgodkännande) som skapats manuellt genom Threat Explorer, eller godkända åtgärder som kommer från automatiserade undersökningar, visas i Åtgärdscenter, som kan nås via den vänstra navigeringen under *Review* >  **Action Center**.
...
Åtgärdscenter visar alla reparationsåtgärder under de senaste 30 dagarna. Åtgärder som vidtas via Explorer visas med samma namn som tillhandahålls av säkerhetsoperationsteamen när reparationen skapades. Åtgärder som vidtas genom automatiserade undersökningar visas med titlar som börjar med den relaterade aviseringen som utlöste undersökningen, t.ex., "Zap e-postkluster ..."
...
1. **Fliken Skicka in e-post**: Det här är antalet e-postmeddelanden som skickas in via Threat Explorer eller automatiska undersökningar som ska åtgärdas. Dessa e-postmeddelanden kan vara:
   ...
   **Användbart:** E-postmeddelanden i följande platser i molnpostlådan kan hanteras och flyttas, dvs alla e-postmeddelanden inom den åtgärdsbara kategorin kan flyttas från en plats till en annan:
  - Inkorg 
  - Skräp  
  - Borttagen mapp
  - Mjuk borttagen mapp
   ...
   **Kan inte aktiveras**: E-postmeddelanden på följande platser kan inte hanteras eller flyttas som en del av e-poståtgärder, dvs e-postmeddelanden i icke-avhjälpbara kategori kan inte flyttas antingen i den icke-avhjälpbara kategorin, eller i avhjälpbara. Platser som inte kan åtgärdas är:

   - Karantän
   - Svår borttagen mapp
   - On-prem / extern
   - Misslyckades /tappade
  
   Skickade meddelanden kategoriseras som antingen avhjälpbara eller som inte kan åtgärdas. I de flesta fall bör reparationsbara och icke-åtgärdbara meddelanden lägga till upp till de totala meddelanden som skickas. Det kan dock finnas sällsynta fall där skickade meddelanden inte kan öka summan av avhjälpbara och icke-avhjälpbara objekt, och kan vara antingen högre eller lägre än det totala antalet skickade meddelanden. Detta kan inträffa på grund av systemfördröjningar, time-outs eller utgångna meddelanden. Meddelanden upphör att gälla baserat på Explorers kvarhållningsperiod för din organisation.
   ...
   Om reparation har fastnat i ett "Pågående" tillstånd ett tag, är det sannolikt på grund av systemfördröjningar. Det kan ta upp till några timmar att åtgärda. Det kan finnas en variant som observerats i antalet postinlämningar eftersom vissa e-postmeddelanden inte var en del av frågan när du påbörjade reparation, på grund av systemfördröjningar. Det är en bra idé att försöka åtgärda i sådana fall.  
...
2. **Fliken Åtgärdsloggar**: På den här fliken visas resultatet av meddelanden som har åtgärdats, inklusive information som godkänt datum, godkännare (administratör som godkände åtgärden), åtgärd, status och antal.  
   ...
   - **Framgång**: När den önskade åtgärden för avhjälpbara e-postmeddelanden utförs, dvs matchar önskad destination plats och avsikten med admin. Till exempel: En administratör vill ta bort e-postmeddelanden från postlådor, så att de vidtar åtgärder för att ta bort e-postmeddelanden. Om ett åtgärdsbart e-postmeddelande hittas i den mjuka borttagna mappen efter att åtgärden har vidtagits visas statusen som lyckad.  
...
Se [Undersöka skadlig e-post som levererades i Office 365](https://docs.microsoft.com/microsoft-365/security/office-365-security/investigate-malicious-email-that-was-delivered?view=o365-worldwide).
