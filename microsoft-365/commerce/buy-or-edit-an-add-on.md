---
title: Köpa eller redigera ett tillägg för Office 365 för företag
f1.keywords:
- NOCSH
ms.author: cmcatee
author: cmcatee-MSFT
manager: mnirkhe
audience: Admin
ms.topic: article
ms.service: o365-administration
localization_priority: Normal
ms.collection:
- M365-subscription-management
- Adm_O365
- Adm_TOC
- commerce
search.appverid:
- BCS160
- MET150
- MOE150
- BEA160
ms.assetid: 4e7b57d6-b93b-457d-aecd-0ea58bff07a6
description: Läs om hur du köper ett tillägg för att lägga till ytterligare funktioner i din Office 365 för företag-prenumeration. Du kan också uppdatera och ta bort ett tillägg.
ms.custom: okr_SMB
ms.openlocfilehash: 78113fcfdfce03ae09318e062847cedd1938760a
ms.sourcegitcommit: 812aab5f58eed4bf359faf0e99f7f876af5b1023
ms.translationtype: MT
ms.contentlocale: sv-SE
ms.lasthandoff: 03/02/2020
ms.locfileid: "42808689"
---
# <a name="buy-or-edit-an-add-on-for-office-365-for-business"></a>Köpa eller redigera ett tillägg för Office 365 för företag

Flera av Office 365 för företag-abonnemangen har tillägg som du kan köpa för din prenumeration. Ett tillägg ger ytterligare funktioner med den prenumeration som du köper det för.


::: moniker range="o365-worldwide"

> [!NOTE]
> Om du inte använder det nya administrationscentret för Microsoft 365 kan du aktivera det genom att välja **Prova det nya administrationscentret** längst upp på startsidan.

::: moniker-end 

## <a name="buy-an-add-on"></a>Köpa ett tillägg

::: moniker range="o365-worldwide"

1. Gå till sidan <a href="https://go.microsoft.com/fwlink/p/?linkid=868433" target="_blank">Faktureringsköpstjänster</a> i **administrationscentret.** \>

2. Välj **Tillägg**längst ned på sidan **Köptjänster** .

3. På sidan **Köptjänster** väljer du den tilläggsprenumeration som du vill köpa.

4. Ange hur många användarlicenser du behöver och om du vill betala varje månad eller för hela året. Välj om du vill att licenser automatiskt ska tilldelas alla användare som inte redan har en licens. Om det visas väljer du en basprenumeration.

5. Välj **Checka ut nu**.

6. Granska prisinformationen och välj sedan **Nästa**.

7. Ange betalningsinformation och välj **sedan Beställ** \> **Gå till Admin Home**.

::: moniker-end

::: moniker range="o365-germany"

1. Gå till sidan **Fakturering** \> <a href="https://go.microsoft.com/fwlink/p/?linkid=847745" target="_blank">Prenumerationer</a> i administrationscentret.

2. På sidan **Prenumerationer** väljer du den prenumeration som du vill köpa ett tillägg för.

3. Välj Tillägg **Köp tillägg**längst ned till vänster i **prenumerationsinformationen** \> .

    ![Köp länken Lägg till på sidan Prenumerationer i administrationscentret för Microsoft 365.](../media/f5cbc3fa-90f7-4299-976d-2482f2c69755.png)
  
4. På sidan **Köptjänster** musar eller trycker du på den tilläggsprenumeration som du vill köpa och väljer sedan **Köp nu**.

5. Ange hur många användarlicenser du behöver och om du vill betala varje månad eller för hela året. Välj om du vill att licenser automatiskt ska tilldelas alla användare som inte redan har en licens. Om det visas väljer du en basprenumeration.

6. Välj **Checka ut nu**.

7. Granska prisinformationen och välj sedan **Nästa**.

8. Ange betalningsinformation och välj **sedan Beställ** \> **Gå till Admin Home**.

::: moniker-end

::: moniker range="o365-21vianet"

1. Gå till sidan **Fakturering** \> <a href="https://go.microsoft.com/fwlink/p/?linkid=850626" target="_blank">Prenumerationer</a> i administrationscentret.

2. På sidan **Prenumerationer** väljer du den prenumeration som du vill köpa ett tillägg för.

3. Välj Tillägg **Köp tillägg**längst ned till vänster i **prenumerationsinformationen** \> .

    ![Köp länken Lägg till på sidan Prenumerationer i administrationscentret för Microsoft 365.](../media/f5cbc3fa-90f7-4299-976d-2482f2c69755.png)
  
4. På sidan **Köptjänster** musar eller trycker du på den tilläggsprenumeration som du vill köpa och väljer sedan **Köp nu**.

5. Ange hur många användarlicenser du behöver och om du vill betala varje månad eller för hela året. Välj om du vill att licenser automatiskt ska tilldelas alla användare som inte redan har en licens. Om det visas väljer du en basprenumeration.

6. Välj **Checka ut nu**.

7. Granska prisinformationen och välj sedan **Nästa**.

8. Ange betalningsinformation och välj **sedan Beställ** \> **Gå till Admin Home**.

::: moniker-end


## <a name="update-an-add-on"></a>Uppdatera ett tillägg

Stegen du använder för att uppdatera ett tillägg beror på om tillägget är ett traditionellt tillägg, eller ett fristående tillägg. 

*Traditionella tillägg* är kopplade till en viss prenumeration. Om du säger upp prenumerationen avbryts även det associerade tillägget.
  
*Fristående tillägg* är inte kopplade till en viss prenumeration. Fristående tillägg visas som en separat prenumeration på sidan **Prenumerationer** (eller på sidan **Produkter & tjänster** i det nya administrationscentret) och har ett eget utgångsdatum. Du hanterar ett fristående tillägg på samma sätt som du hanterar andra prenumerationer.
  
### <a name="update-a-traditional-add-on"></a>Uppdatera ett traditionellt tillägg

::: moniker range="o365-worldwide"

1. I administrationscentret går du till sidan **Fakturering** \> <a href="https://go.microsoft.com/fwlink/p/?linkid=842054" target="_blank">Produkter och tjänster</a>.

2. På sidan **Produkter & tjänster** väljer du den prenumeration som du vill uppdatera ett tillägg för.

3. Välj **tillägg**. De tillägg som du har köpt visas.

4. Välj **Ändra kvantitet** för det tillägg som du vill ändra.

5. Ange den kvantitetsändring du vill ha för prenumerationen och välj sedan **Skicka ändring**.

::: moniker-end


::: moniker range="o365-germany"

1. Gå till sidan **Fakturering** \> <a href="https://go.microsoft.com/fwlink/p/?linkid=847745" target="_blank">Prenumerationer</a> i administrationscentret.

2. På sidan **Prenumerationer** väljer du den prenumeration som du vill uppdatera ett tillägg för.

3. Välj **tillägg**.

    Tilläggen du har köpt visas och under varje tillägg visas länken **Ändra antal**.

4. Välj länken **Ändra kvantitet** för det tillägg som du vill uppdatera. 

    ![Länkarna Tilläggsprogram och Ändra antal.](../media/fa7c39f4-5256-49bb-9a26-70f1a52ae926.png)
  
5. Ange det antal användarlicenser som du behöver i rutan och välj sedan **Skicka**.

    > [!TIP]
    > Du kan även använda upp- och nedpilen när du vill ändra antalet användarlicenser eller bara ange siffran du vill ha i rutan.
  
    ![Rutan Totalt antal licenser och knappen Skicka i dialogrutan Lägg till eller ta bort licenser.](../media/dfee31e0-092d-4565-9bf5-ff3909b771e1.png)

::: moniker-end

::: moniker range="o365-21vianet"

1. Gå till sidan **Fakturering** \> <a href="https://go.microsoft.com/fwlink/p/?linkid=850626" target="_blank">Prenumerationer</a> i administrationscentret.

2. På sidan **Prenumerationer** väljer du den prenumeration som du vill uppdatera ett tillägg för.

3. Välj **tillägg**.

    Tilläggen du har köpt visas och under varje tillägg visas länken **Ändra antal**.

4. Välj länken **Ändra kvantitet** för det tillägg som du vill uppdatera. 

    ![Länkarna Tilläggsprogram och Ändra antal.](../media/fa7c39f4-5256-49bb-9a26-70f1a52ae926.png)
  
5. Ange det antal användarlicenser som du behöver i rutan och välj sedan **Skicka**.

    > [!TIP]
    > Du kan även använda upp- och nedpilen när du vill ändra antalet användarlicenser eller bara ange siffran du vill ha i rutan.
  
    ![Rutan Totalt antal licenser och knappen Skicka i dialogrutan Lägg till eller ta bort licenser.](../media/dfee31e0-092d-4565-9bf5-ff3909b771e1.png)


::: moniker-end

### <a name="update-a-standalone-add-on"></a>Uppdatera ett fristående tillägg

::: moniker range="o365-worldwide"

1. I administrationscentret går du till sidan **Fakturering** \> <a href="https://go.microsoft.com/fwlink/p/?linkid=842054" target="_blank">Produkter och tjänster</a>.

2. På sidan **Produkter & tjänster** väljer du den tilläggsprenumeration som du vill uppdatera och väljer sedan Lägg **till/ta bort licenser**.

3. Ange det antal licenser som du behöver i rutan och välj sedan **Skicka ändring**.

::: moniker-end

::: moniker range="o365-germany"

1. Gå till sidan **Fakturering** \> <a href="https://go.microsoft.com/fwlink/p/?linkid=847745" target="_blank">Prenumerationer</a> i administrationscentret.

2. På sidan **Prenumerationer** väljer du den tilläggsprenumeration som du vill uppdatera och väljer sedan **Lägg till/ta bort licenser**.

3. Ange det antal licenser som du behöver i rutan och välj sedan **Skicka**.

    > [!TIP]
    > Du kan även använda upp- och nedpilen för att ändra antalet användarlicenser eller ange siffran du vill ha i rutan.
  
    ![Rutan Totalt antal licenser och knappen Skicka i dialogrutan Lägg till eller ta bort licenser.](../media/dfee31e0-092d-4565-9bf5-ff3909b771e1.png)

::: moniker-end

::: moniker range="o365-21vianet"

1. Gå till sidan **Fakturering** \> <a href="https://go.microsoft.com/fwlink/p/?linkid=850626" target="_blank">Prenumerationer</a> i administrationscentret.

2. På sidan **Prenumerationer** väljer du den tilläggsprenumeration som du vill uppdatera och väljer sedan **Lägg till/ta bort licenser**.

3. Ange det antal licenser som du behöver i rutan och välj sedan **Skicka**.

    > [!TIP]
    > Du kan även använda upp- och nedpilen för att ändra antalet användarlicenser eller ange siffran du vill ha i rutan.
  
    ![Rutan Totalt antal licenser och knappen Skicka i dialogrutan Lägg till eller ta bort licenser.](../media/dfee31e0-092d-4565-9bf5-ff3909b771e1.png)

::: moniker-end


## <a name="remove-an-add-on"></a>Ta bort ett tillägg

Du kan avsluta ett fristående tillägg på samma sätt som du [avbryter en prenumeration](subscriptions/cancel-your-subscription.md). Du kan även avaktivera **automatisk förnyelse** för ett fristående tillägg med hjälp av stegen ovan. Du kan inte dock ta bort ett traditionellt tillägg efter att det har köpts. Om du behöver ta bort ett traditionellt tillägg, [vänligen kontakta supporten för hjälp](../admin/contact-support-for-business-products.md).
  
## <a name="available-add-ons"></a>Tillgängliga tillägg

I följande tabell visas tillägg som är tillgängliga för olika versioner av Office 365 för företag.
  
Information om tillägg till Skype för företag finns i [tilläggslicensiering för Skype för företag och Microsoft Teams](https://docs.microsoft.com/SkypeForBusiness/skype-for-business-and-microsoft-teams-add-on-licensing/skype-for-business-and-microsoft-teams-add-on-licensing).
  
****

|**Tillägg**|**Tillgängliga i de här prenumerationerna**|
|:-----|:-----|
|Customer Lockbox  <br/> | Office 365 Enterprise E1  <br/>  Office 365 Enterprise, E3  <br/>  Office 365 Enterprise E4  <br/> |
|[Avancerad hotskydd för Office 365 Online](https://go.microsoft.com/fwlink/p/?LinkId=691156) <br/> | Office 365 Enterprise E1  <br/>  Office 365 Enterprise, E3  <br/>  Office 365 Enterprise E4  <br/> |
|[Exchange Online - arkivering](https://go.microsoft.com/fwlink/p/?LinkId=691157) <br/> | Office 365 Business Essentials  <br/>  Office 365 Business Premium  <br/>  Office 365 Enterprise E1  <br/>  Office 365 Enterprise F1  <br/> |
|Microsoft MyAnalytics  <br/> | Office 365 Enterprise E1  <br/>  Office 365 Enterprise, E3  <br/>  Office 365 Enterprise E4  <br/> |
|Office 365 Advanced Compliance  <br/> | Office 365 Enterprise E1  <br/>  Office 365 Enterprise, E3  <br/>  Office 365 Enterprise E4  <br/> |
|Office 365 Extra File Storage  <br/> | Office 365 Business  <br/>  Office 365 Business Essentials  <br/>  Office 365 Business Premium  <br/>  Office 365 Enterprise E1  <br/>  Office 365 Enterprise, E2  <br/>  Office 365 Enterprise E3  <br/>  Office 365 Enterprise E4  <br/>  Office 365 Enterprise E5  <br/>  Office för webben med SharePoint Plan 1  <br/>  Office för webben med SharePoint Plan 2  <br/>  SharePoint Online (abonnemang 1)  <br/>  SharePoint Online (abonnemang 2)  <br/> |
|Telefonsystemet i Office 365  <br/> | Office 365 Enterprise E1  <br/>  Office 365 Enterprise, E3  <br/>  Office 365 Enterprise E4  <br/>  Office 365 Enterprise E5  <br/> |
|Ljudkonferens i Office 365  <br/> |  Office 365 Business Essentials <br/> Office 365 Business Premium <br/> Office 365 Enterprise E1 <br/> Office 365 Enterprise, E3 <br/> Office 365 Enterprise E4 <br/> Office 365 Enterprise E5 utan ljudkonferenser <br/> Microsoft 365 Företag <br/> Microsoft 365 E3 <br/> Microsoft 365 E5  <br/> |
|Samtalspoäng i Office 365  <br/> | Office 365 Enterprise E5  <br/> |
|Abonnemang för inrikessamtal i Office 365  <br/> | Office 365 Enterprise E5  <br/> |
|Samtalsabonnemang för internationella samtal i Office 365  <br/> | Office 365 Enterprise E5  <br/> |
  
## <a name="related-articles"></a>Relaterade artiklar

[Lägga till lagringsutrymme för prenumerationen](add-storage-space.md)
