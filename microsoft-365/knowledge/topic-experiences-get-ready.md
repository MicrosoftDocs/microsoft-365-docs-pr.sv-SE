...
2. [Anslut information till Microsoft Graph](#2-connect-information-to-microsoft-graph)
    ...
    - För innehåll som inte kan flyttas kan du använda Graph-kopplingar för att förbättra sökningen och förbereda för framtida inkludering.
...
4. [Skydda innehåll på rätt sätt](#4-secure-content-appropriately)
    ...
    - Granska konfigurationen av Microsoft Graph användare och innehåll:
...
- [SharePoint Migreringsverktyget (SPMT)](/sharepointmigration/introducing-the-sharepoint-migration-tool)
...
- Migrera till en modern webbplats – som innehåller Microsoft Teams. Indexeringen kan ske på alla SharePoint webbplatser (klassisk eller modern), och ämnen som visas för användare med markeringar och kort händer bara på moderna sidor.
...
## <a name="2-connect-information-to-microsoft-graph"></a>2. Anslut information till Microsoft Graph
...
- Uppmuntra webbplatsägare att granska delning och behörigheter.
  ...
  SharePoint webbplatsägare kan granska en delningsrapport för sin webbplats med fullständig information om alla behörigheter och delningslänkar som konfigurerats på webbplatsen finns [i Dela rapporter](/sharepoint/sharing-reports). Här listas interna och externa (gäst)användare.

  Webbplatsägare kan också se vem som har  behörighet för webbplatsen genom att gå till sidorna Webbplatsbehörigheter **och Inställningar** behörigheter.

  1. På din webbplats väljer du **Inställningar**  >  **Webbplatsbehörigheter**. Kontrollera vem som visas under Webbplatsägare, Webbplatsmedlemmar och Webbplatsbesökare. Kontrollera om det finns gästanvändare.
  2. På sidan **Behörigheter** väljer du **Avancerade behörigheter Inställningar**. Du kan söka efter unika behörigheter och se vem som har begränsad åtkomst till objekt på webbplatsen.

- Granska Microsoft 365 grupper och Teams för att se till att de är korrekt inställda som offentliga eller privata grupper. Nya Teams och Microsoft 365 är inställda på privata som standard, men när de först släpptes offentliga som standard. Om du redan var adopter av dessa tekniker kanske du vill granska. Dessutom utvecklas en grupps funktion ofta under hela livscykeln, och inställningen kan behöva uppdateras så att den återspeglar den aktuella användningen av gruppen.
...
Hantering av ämnen omfattar tre viktiga roller, inklusive två nya AAD-roller (Azure Active Directory): Knowledge Administrator och Knowledge Manager:
