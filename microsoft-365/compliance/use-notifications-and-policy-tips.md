...
Du kan använda en DLP-princip (Data Loss Prevention) för att identifiera, övervaka och skydda känslig information i hela Office 365. Du vill att alla organisationen som arbetar med den här känsliga informationen ska uppfylla reglerna som följer DLP-principerna, men du vill heller inte skapa blockeringar i onödan som gör att de inte får jobbet gjort. Det är här e-postaviseringar och principtips kan vara till hjälp.
...
- Visa ett principtips för innehåll som står i konflikt med DLP-principen:
  ...
  - För e-post i Outlook på webben, Outlook 2013 och senare visas principtipset högst upp i meddelandet ovanför mottagarna medan meddelandet består.
    
  - För dokument på ett OneDrive för företag-konto eller en SharePoint Online-webbplats visas principtipset med en varningsikon som visas för objektet. Om du vill visa mer information kan  du markera ett objekt och sedan välja ikonen för informationsfönstret i det övre högra hörnet på sidan för ![ att öppna ](../media/50b6d51b-92b4-4c5f-bb4b-4ca2d4aa3d04.png) informationsfönstret. 
    
  - För Excel-, PowerPoint- och Word-dokument som lagras på en OneDrive för företag-webbplats eller en SharePoint Online-webbplats som ingår  i DLP-principen visas principtipset i meddelandefältet och i Backstage-vyn (Arkiv-menyn). \> 
    
## <a name="add-user-notifications-to-a-dlp-policy"></a>Lägga till användarmeddelanden i en DLP-princip

När du skapar en DLP-princip kan du aktivera **användarmeddelanden.** När användarmeddelanden är aktiverade skickar Microsoft 365 ut både e-postaviseringar och principtips. Du kan anpassa vilka aviseringar som skickas till, e-posttexten och policytipstexten.
...
6. Välj platser som du vill att DLP-principen ska skydda genom att göra något av följande:
   ...
   Om du vill inkludera eller exkludera en hel plats, till exempel all Exchange-e-post eller alla OneDrive-konton, kan du slå på eller stänga av **Status** för den platsen. 
    
   Om du bara vill inkludera vissa SharePoint-webbplatser eller OneDrive-konton, växlar du status till på och klickar sedan på länkarna **under** Inkludera för att välja specifika webbplatser eller konton.  
...
|**Om DLP-principregeln gör det här...**|**Standardaviseringen för SharePoint- eller OneDrive för företag-dokument innehåller följande...**|**Då visas följande i standardmeddelandet för Outlook-meddelanden:**|
|:-----|:-----|:-----|
...
|%%ContentURL%%  <br/> |URL-adressen till dokumentet på SharePoint Online-webbplatsen eller OneDrive för företag-webbplatsen.  <br/> |
...
## <a name="policy-tips-on-onedrive-for-business-sites-and-sharepoint-online-sites"></a>Principtips på OneDrive för företag-webbplatser och SharePoint Online-webbplatser

När ett dokument på en OneDrive för företag- eller SharePoint Online-webbplats matchar en regel i en DLP-princip och den regeln använder principtips, visar principtipsen särskilda ikoner i dokumentet:
...
## <a name="policy-tips-in-outlook-on-the-web-and-outlook-2013-and-later"></a>Principtips i Outlook på webben, Outlook 2013 och senare

När du skriver ett nytt e-postmeddelande i Outlook på webben, Outlook 2013 och senare visas ett principtips om du lägger till innehåll som matchar en regel i en DLP-princip, och den regeln använder principtips. Principtipset visas högst upp i meddelandet, ovanför mottagarna, medan meddelandet består.
...
### <a name="outlook-2013-and-later-supports-showing-policy-tips-for-only-some-conditions"></a>Outlook 2013 och senare har stöd för visning av principtips endast för vissa villkor

För närvarande stöder Outlook 2013 och senare visning av principtips endast för dessa villkor:
...
### <a name="policy-tips-in-the-exchange-admin-center-vs-the-security-amp-compliance-center"></a>Principtips i administrationscentret för Exchange och &amp; Säkerhetsefterlevnadscenter

Principtips kan antingen fungera med DLP-principer och e-postflödesregler som skapats i Exchange admin center eller med DLP-principer som skapats i Säkerhetsefterlevnad, &amp; men inte båda. Det beror på att dessa principer lagras på olika platser, men principtips kan endast ritas från en enda plats.
  
Om du har konfigurerat principtips i Administrationscenter för Exchange visas inte principtips som du konfigurerar i säkerhetsefterlevnadscentret för användare i Outlook på webben och Outlook 2013 och senare förrän du inaktiverar tipsen i &amp; Exchange admin center. Det säkerställer att dina aktuella Exchange-e-postflödesregler (kallas även transportregler) fortsätter att fungera tills du väljer att växla över till &amp; Säkerhetsefterlevnadscenter.
  
Observera att även om principtips bara kan ritas från en enda plats skickas alltid e-postaviseringar, även om du använder DLP-principer i både Säkerhetsefterlevnadscenter och &amp; Administrationscenter för Exchange.
...
När personer arbetar med känsligt innehåll i skrivbordsversionerna av Excel, PowerPoint och Word kan principtips meddela dem i realtid om att innehållet står i konflikt med en DLP-princip. Detta kräver att:
  
- Office-dokumentet lagras på en OneDrive för företag-webbplats eller SharePoint Online-webbplats.
    
- Webbplatsen ingår i en DLP-princip som är konfigurerad för att använda principtips.
    
Office-program synkroniserar automatiskt DLP-principer direkt från Office 365 och skannar sedan dokumenten för att säkerställa att de inte står i konflikt med DLP-principerna och visa principtips i realtid.

> [!NOTE]
> Office-skrivbordsappar söker igenom dokumenten själva för att avgöra om DLP-principtips ska visas. De visar inte principtips som SharePoint Online-webbplatser eller OneDrive för företag-webbplatser redan har fastställt ska visas i en fil. Därför visas kanske inte alltid ett DLP-principtips i skrivbordsapparna som visas på SharePoint Online-webbplatserna eller i OneDrive för företag-webbplatserna. Office-programmen på webben visar däremot bara DLP-principtips som SharePoint Online-webbplatser eller OneDrive för företag-webbplatser redan har fastställt ska visas.
...
![Alternativ för principtips i Backstage i Excel 2016](../media/5b3857ba-907e-456e-ae43-888b594c049c.png)
  
I alla Office-program kan man välja att inaktivera principtips. Om de är inaktiverade visas inte principtips som är enkla meddelanden i meddelandefältet eller Backstage-vyn (på **fliken** Arkiv). Principtips om blockering och åsidosättning visas dock fortfarande, och de kommer fortfarande att få e-postaviseringen. När principtips inaktiveras undantas inte dokumentet från de DLP-principer som har tillämpats på det. 
